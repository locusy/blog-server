...
module.exports = {
    async list(ctx, next) {
        ...
        let { type = "JavaScript", pageindex = 1, pagesize = 5} = ctx.request.query;
        console.log('type:'+type+','+'pageindex:'+pageindex +','+ 'pagesize:'+pagesize)
        try {
            
            let data = await ctx.find(blogModel, {}, null, 
                {limit: pagesize*1, skip: (pageindex-1)*pagesize, sort: {level: -1, createTime: -1}});
